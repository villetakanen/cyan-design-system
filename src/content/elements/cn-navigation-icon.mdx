...
<div class="flex flex-row flex-no-wrap">
  ...
  <div class="border border-radius p-2 grow">
    <cn-toggle-button id="toggleActive" label="Active"></cn-toggle-button>
  </div>
</div>

<script>{`document.getElementById('toggleActive').addEventListener('change', (event) => {console.log(event.target.pressed);document.querySelectorAll('cn-navigation-icon').forEach((icon) => {icon.active = event.target.pressed});});`}</script>
